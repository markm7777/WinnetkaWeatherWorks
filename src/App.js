...
class MyWeatherComponent extends React.Component {
  ...
  getWeather = () => {
    ...
    fetch(url)
      ...
      .then((json) => {
        selectedData = json.data.slice(0, 7).map((item) => {
          return {
            ...
            day: new Date(item.datetime).toDateString().substr(0, 3),
        ...
        this.setState({
          weatherForecastData: selectedData,
          returnedCityState:
            json.city_name +
            ', ' +
            (this.isNumeric(json.state_code)
              ? json.country_code
              : json.state_code) +
            ' (' +
            json.timezone +
      ...
      .catch((error) =>
        this.setState({
          error: `Error locating forecast for: ${this.state.city}`,
        })
      );
  };
  ...
  render() {
    ...
    //commenting out today and days since they are never used
    return (
      <div id='mainDiv'>
        ...
        ></input>
        <div className='divider' />
        <button id='getWeatherButton' onClick={this.getWeather}>
          Get 7-day Forecast
        </button>
            ...
            return (
              <div key={index} id='forecastDayDiv'>
                ...
                <div>
                  <img
                    src={
                      process.env.PUBLIC_URL + '/icons/' + item.icon + '.png'
                    }
                    alt='Smiley face'
                    height='80'
                    width='80'
                ...
                <div>{item.maxTemp + '\xB0' + '/' + item.minTemp + '\xB0'}</div>
